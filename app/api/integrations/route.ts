...
const saveSchema = z.object({
  ...
  type: z.enum(['meta', 'activecampaign', 'mailchimp', 'stripe', 'google_ads', 'gohighlevel']),
...
export async function GET() {
  ...
  // Get user's workspace first — filter integrations by workspace (security fix)
  const { data: workspace } = await serviceClient
    .from('workspaces')
    .select('id')
    .eq('user_id', user.id)
    .single();
  if (!workspace) return NextResponse.json({ integrations: [] });
...
export async function PATCH(req: NextRequest) {
  ...
  if (integration.type === 'activecampaign') {
  ...
  } else if (integration.type === 'google_ads') {
    // Test Google Ads connection by verifying the developer token header is accepted.
    // We attempt a lightweight call to the Google Ads API (customer info endpoint).
    const axios = (await import('axios')).default;
    try {
      ...
      const res = await axios.get(
        ...
        {
          headers: {
            Authorization: `Bearer ${config.access_token || config.developer_token}`,
            'developer-token': config.developer_token,
            'Content-Type': 'application/json',
          },
        }
      );
      const name = (res.data as Record<string, unknown>)?.resourceName || `Customer ${config.customer_id}`;
      result = { success: true, accountName: String(name) };
    } catch (err: unknown) {
    ...
    // Test GoHighLevel connection by fetching the location info
    const axios = (await import('axios')).default;
    try {
      const res = await axios.get(
        `https://services.leadconnectorhq.com/locations/${config.location_id}`,
        {
          headers: {
      ...
      const locName = (res.data as Record<string, unknown>)?.name || config.location_id;
      result = { success: true, accountName: `GoHighLevel: ${locName}` };
    } catch (err: unknown) {
      const e = err as { response?: { data?: unknown }; message?: string };
      const detail = e.response?.data
        ? JSON.stringify(e.response.data).slice(0, 200)
